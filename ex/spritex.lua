...
function spritex:reset_scale9(w,h, spr)
    ...
    if not self.__scale9 then
        ...
        self.__scale9x = x2
        self.__scale9y = y2
        _,_,x2,y2 = p:fetch_by_index(4):aabb()
        self.__scale9w = x2
        self.__scale9h = y2
        _,_,x2,y2 = p:fetch_by_index(8):aabb()
        self.__scale9x2 = x2
        self.__scale9y2 = y2
        self.__scale9 = true
    end
    ...
    c = p:fetch_by_index(1)
    c:sr(scalex, 1)
    c:ps(scale9x, 0)
    c = p:fetch_by_index(2)
    c:ps(scale9x+cw, 0)

    c = p:fetch_by_index(3)
    c:sr(1, scaley)
    c:ps(0, scale9y)
    c = p:fetch_by_index(4)
    c:sr(scalex, scaley)
    c:ps(scale9x, scale9y)
    c = p:fetch_by_index(5)
    c:sr(1, scaley)
    c:ps(scale9x+cw, scale9y)

    c = p:fetch_by_index(6)
    c:ps(0, scale9y+ch)
    c = p:fetch_by_index(7)
    c:sr(scalex, 1)
    c:ps(scale9x, scale9y+ch)
    c = p:fetch_by_index(8)
